...
docker run -d -p 8888:8888 -e SHELL=bash -v ${SCRIPTPATH}/:/main -v ${SCRIPTPATH}/jupyter_settings/ipython_config/${SETTING_DIR_NAME}/ipython_config.py:/root/.ipython/profile_default/ipython_config.py -v ${SCRIPTPATH}/jupyter_settings/jupyter_lab_config/${SETTING_DIR_NAME}/jupyter_lab_config.py:/root/.jupyter/jupyter_notebook_config.py -v ${SCRIPTPATH}/notebooks:/tf/notebooks:z -v ${SCRIPTPATH}/jupyter_settings/user-settings/${SETTING_DIR_NAME}:/root/.jupyter/lab/user-settings/@jupyterlab -e GRANT_SUDO=yes --shm-size=${SHAREDMEM} --user root --name ganhairdata --privileged yuryueng/ganhackertonbaseline
echo "Finished"
